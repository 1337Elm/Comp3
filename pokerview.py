...
class Window(QGraphicsView):
    ...
    def Buttons(self,action,player,OtherPlayer):
        ...
        if self.game.turns() == 2:
            ...
            if self.action[-1] == "Check":
                ...
                if self.game.turns() == 5 or self.game.turns() == 4:
                    self.Check.clicked.connect(self.FourthCard)
                elif self.game.turns() == 7 or self.game.turns() == 6:
                    self.Check.clicked.connect(self.River)
                elif self.game.turns() == 9 or self.game.turns() == 8:
                    ...
                    self.Check.clicked.connect(self.updateMoney)
                    self.Another = QPushButton("Play another round",self)
                    self.Another.setFixedWidth(200)
                    self.Another.clicked.connect(self.playAnotherRound)
                    self.vbox.addWidget(self.Another)
                else:
                ...
                if "All in" in self.action:
                    ...
                    self.FinnishRound = QPushButton("See who wins",self)
                    self.FinnishRound.setFixedWidth(200)
                    if self.game.turns() == 4 or self.game.turns() == 5:
                        self.FinnishRound.clicked.connect(self.CardsOnBoard)
                        self.FinnishRound.clicked.connect(self.FourthCard)
                        self.FinnishRound.clicked.connect(self.River)
                        self.FinnishRound.clicked.connect(self.showWinner)
                        self.FinnishRound.clicked.connect(self.updateMoney)
                        self.FinnishRound.clicked.connect(lambda: self.game.determineWinner(player,OtherPlayer))
                        self.vbox.addWidget(self.FinnishRound)
                        self.Another = QPushButton("Play another round",self)
                        self.Another.setFixedWidth(200)
                        self.Another.clicked.connect(self.playAnotherRound)
                        self.vbox.addWidget(self.Another)
                    elif self.game.turns() == 6 or self.game.turns() == 7:
                        self.FinnishRound.clicked.connect(self.FourthCard)
                        self.FinnishRound.clicked.connect(self.River)
                        self.FinnishRound.clicked.connect(self.showWinner)
                        self.FinnishRound.clicked.connect(self.updateMoney)
                        ...
                        self.Another.clicked.connect(self.playAnotherRound)
                        self.vbox.addWidget(self.Another)
                    elif self.game.turns() == 8 or self.game.turns() == 9:
                        self.FinnishRound.clicked.connect(self.River)
                        self.FinnishRound.clicked.connect(self.showWinner)
                        self.FinnishRound.clicked.connect(self.updateMoney)
                        self.FinnishRound.clicked.connect(lambda: self.game.determineWinner(player,OtherPlayer))
                    ...
                    self.Deal1 = QPushButton("Deal",self)
                    self.Deal1.setFixedWidth(200)
                    if self.game.turns() == 6 or self.game.turns() == 5:
                        self.Deal1.clicked.connect(self.FourthCard)
                        self.Deal1.clicked.connect(lambda: self.OpponentButtons(self.action.append("Deal"),player,OtherPlayer))
                    elif self.game.turns() == 8 or self.game.turns() == 7:
                        self.Deal1.clicked.connect(self.River)
                        self.Deal1.clicked.connect(lambda: self.OpponentButtons(self.action.append("Deal"),player,OtherPlayer))
                    elif self.game.turns() == 10 or self.game.turns() == 9:
                        ...
                        self.Deal1.clicked.connect(self.updateMoney)
                        self.Another = QPushButton("Play another round",self)
                        self.Another.setFixedWidth(200)
                        self.Another.clicked.connect(self.playAnotherRound)
                        self.vbox.addWidget(self.Another)
    ...
    def playAnotherRound(self):
        """Starts the next round if both players still have money
        
        :param self: the window object
        :type self: object
        """
        if self.player1.Money > 0 and self.player2.Money > 0:
            self.vbox.removeWidget(self.label1)
            self.label1.deleteLater()
            self.vbox.removeWidget(self.label2)
            self.label2.deleteLater()
            self.vbox.removeWidget(self.label3)
            self.label3.deleteLater()
            self.vbox.removeWidget(self.Another)
            self.Another.deleteLater()
            self.vbox.removeWidget(self.Check)
            self.Check.deleteLater()
            self.vbox.removeWidget(self.See)
            self.See.deleteLater()
            ...
            self.player1.LastBet = 0
            self.player2.LastBet = 0
            self.action.clear()
            self.game.roundCounter += 1
            self.resetBoard()
            self.game.Turn = 0
            self.initGame()
        else:
